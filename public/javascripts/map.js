...
//drawLine event listners
var drawLineMouseDown = function(e){
    console.log(isDrawing)
    isDrawing = true;
    console.log("isDrawing after call:"+isDrawing);
...
var drawLineMouseMove = function(){
    if (!isDrawing) {
        return;
    };
    const pos = stage.getPointerPosition();
...
function drawLine(color, width){
    //Remove all event listeners from stage before adding new ones
    //stage.removeEventListener('mousedown touchstart')
    console.log("in drawLine");
    // All functions reference the stage but write to the map layer
    stage.on('mousedown touchstart', drawLineMouseDown);
    stage.on('mouseup touchend', drawLineMouseUp);
    stage.on('mousemove touchmove', drawLineMouseMove);
